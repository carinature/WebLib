...
'''	this takes numbers as they are usually printed in indices(e.g., 1-4, 35-7, 109-11) and expands 
    ...
    if "step" not in request.form:
      ...
      if option=="and":
        ...
        data = data.astype(str).groupby('work').agg({'subject 1':'; '.join, 'subject 2':'; '.join, 'page 1':','.join, 'page 2':','.join, 'ref':','.join, 'author':'first', 'book 1 info':'first','book 2 info':'first',}).reset_index()

        data = data.groupby(["author","work","subject 1","book 1 info","page 1","book 2 info","page 2","ref"])["subject 2"].apply(lambda x: '; '.join(x)).reset_index()
        data = data.groupby(["work","subject 1","subject 2","book 1 info","page 1","book 2 info","page 2","ref"])["author"].apply(lambda x: ', '.join(x)).reset_index()
        data = data.groupby(["author","subject 1","subject 2","book 1 info","page 1","book 2 info","page 2","ref"])["work"].apply(lambda x: ', '.join(x)).reset_index()
        data = data.groupby(["author","work","subject 1","subject 2","page 1","book 2 info","page 2","ref"])["book 1 info"].apply(lambda x: ', '.join(x)).reset_index()
        data = data.groupby(["author","work","subject 1","subject 2","book 1 info","book 2 info","page 2","ref"])["page 1"].apply(lambda x: ', '.join(x)).reset_index()
        data = data.groupby(["author","work","subject 1","subject 2","book 1 info","book 2 info","page 1","ref"])["page 2"].apply(lambda x: ', '.join(x)).reset_index()
        data = data.groupby(["author","work","subject 1","subject 2","book 1 info","page 1","book 2 info","page 2"])["ref"].apply(lambda x: ', '.join(x)).reset_index()
        data = data.groupby(["author","work","subject 1","subject 2","page 1","book 2 info","page 2","ref"])["book 1 info"].apply(lambda x: ', '.join(x)).reset_index()
		
        data = data[["author","work","ref","subject 1","subject 2","book 1 info","page 1","book 2 info","page 2"]]
      ...
      if len(data.index)==0:
        ...
        data["ref"] = data["ref"].apply(rehyph)
        if option!="and":
          data = data.groupby(["author", "work", "subject",'book bibliographic info', 'ref'])['page'].apply(lambda x: ', '.join(x)).reset_index()
        ...
        if fulltext != "f":
          if (len(data)<100) and (len(data!=0)):
            ...
            if  option!="and":
              data["results"] = data["author"]+", <i>"+data["work"]+"</i>, "+data["ref"]+SIM_SUBJECT_TEXT_TAG\
					+data["subject"]+BOOK_TAG_TEXT+data["book bibliographic info"]+" on pages: "+data["page"]+\
					"<table><tr><td style='min-width:600px'>"+data["refcite"]+"</td><td>"+data["refengcite"]+"</td></tr></table>"
              data = data[['results']]
            if  option=="and":
					data["results"] = data["author"]+", <i>"+data["work"]+"</i>, "+data["ref"]+SIM_SUBJECT_TEXT_TAG\
            ...
            data.rename(columns={"refcite": "full text.............................................................................."}, inplace=True)
            data.rename(columns={"refengcite": "English translation.............................................................................."}, inplace=True)
          if len(validated)!=0:
            ...
            if  option!="and":
              validated["results"] = validated["author"]+", <i>"+validated["work"]+"</i>, "+validated["ref"]\
				...
				+", pages: "+validated["page"]+"</span><table><tr><td style='min-width:600px'>"+validated\
				["refcite"]+"</td><td>"+validated["refengcite"]+"</td></tr></table>"
				validated = validated[['results']]
            if  option=="and":
              validated["results"] = validated["author"]+", <i>"+validated["work"]+"</i>, "+validated["ref"]\
				+SIM_SUBJECT_TEXT_TAG+validated["subject 1"]+"<br>"+validated["subject 2"]+"<br><br>"
            ...
            validated.rename(columns={"refcite": "full text.............................................................................."}, inplace=True)
            validated.rename(columns={"refengcite": "English translation.............................................................................."}, inplace=True)
          if len(highly_validated)!=0:
            ...
            if  option!="and":
              ...
              highly_validated["results"] = highly_validated["Index1"].astype(str)+". "+highly_validated["author"]\
				...
				highly_validated["book bibliographic info"]+", pages: "+highly_validated["page"]+"</span>"
            ...
            highly_validated.rename(columns={"refcite": "full text.............................................................................."}, inplace=True)
            highly_validated.rename(columns={"refengcite": "English translation.............................................................................."}, inplace=True)
          ...
          if option!="and":
            ...
            if len(validated)!=0:
              ...
              validated["results"] = "<b>&emsp;"+validated["Index1"].astype(str)+". "+validated["author"]+", <i>"+validated["work"]+"</i>,\
					"+validated["ref"]+"</b>"+SUBJECT_TAG_TEXT+validated["subject"]\
					+BOOK_TAG_TEXT+validated["book bibliographic info"]+", pages: "+validated["page"]+"</span>"
              validated = validated[['results']]
            if len(highly_validated)!=0:
              if len(highly_validated)<50:
                try:
                  ...
                  highly_validated["results"] = "<b>&emsp;"+highly_validated["Index1"].astype(str)\
					+". "+highly_validated["author"]+", <i>"+highly_validated["work"]\
					+"</i>, "+highly_validated["ref"]\
					+"</b>"+SUBJECT_TAG_TEXT+highly_validated["subject"]+BOOK_TAG_TEXT\
					"+highly_validated["book bibliographic info"]+", pages: "\
					+highly_validated["page"]+
                  ...
                  highly_validated["results"] = "<b>&emsp;"+highly_validated["Index1"].astype(str)+	\
					...
					highly_validated["ref"]+"</b>"+SUBJECT_TAG_TEXT+highly_validated["subject"]\
            ...
            if len(highly_validated)!=0:
              ...
              highly_validated["Index1"]=highly_validated["Index1"]+1
              highly_validated["results"] = highly_validated["Index1"].astype(str)+". "+highly_validated["author"]\
					+", <i>"+highly_validated["work"]+"</i>, "+highly_validated["ref"]+SIM_SUBJECT_TEXT_TAG\
      ...
      return render_template('index.html', ccs = ccs, ccs1 = ccs1, tablev=validated.to_html(escape=False, index=False), \
		step="choose_upper", l = l, tablev1=highly_validated.to_html(escape=False, index=False), lengthvv1 = length_highly_validated, \
		tablek = data.to_html(escape=False), YYY = YYY, fulltext=fulltext, cs=cs, ce=ce, lang=lang,author=author, \
